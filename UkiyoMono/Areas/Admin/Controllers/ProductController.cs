...
public void OptimizarImagen(string rutaImagen)
{
    using (var input = File.OpenRead(rutaImagen))
    using (var original = SKBitmap.Decode(                                                        ))
    {
        var resized = original.Resize(new SKImageInfo(800, 600), SKFilterQuality.High);
        using (var image = SKImage.FromBitmap(resized))
        using (var output = File.OpenWrite("ruta/imagen_optimizada.jpg"))
        {
            image.Encode(SKEncodedImageFormat.Jpeg, 80).SaveTo(output);
				...
				Carga de imagenes tal cual al repo sin optimizar
				if (files != null)
				{
					foreach (IFormFile file in files)
					{
