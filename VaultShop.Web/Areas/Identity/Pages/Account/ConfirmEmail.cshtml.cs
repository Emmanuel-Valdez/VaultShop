...
namespace UkiyoDesignsWeb.Areas.Identity.Pages.Account
{
    public class ConfirmEmailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(string userId, string code)
        {
            ...
            if (user == null)
            {
                return NotFound($"{_localizer["UserUnableToLoad"].Value} '{userId}'.");
            }

            code = Encoding.UTF8.GetString(WebEncoders.Base64UrlDecode(code));
            var result = await _userManager.ConfirmEmailAsync(user, code);
            StatusMessage = result.Succeeded ? _localizer["EmailConfirmed"].Value : _localizer["EmailConfirmedError"].Value;
            return Page();
        }
    }
}
